...
| A12 | Physical vehicle geometry (motorcycle 2.2 m, car 4.3 m, bus 12.0 m, truck 10.0 m) and forced-gap driving behaviour (tau 0.8–1.0; impatience 0.7–1.0; jmIgnoreFoeProb 0.4 motorcycle / 0.25 car / 0.15 heavy; jmTimegapMinor 0.8–1.0) | the earlier encoding set length = PCU × car-space, giving buses and trucks 24.7 m and inflating queue storage ~35% — diagnosed cause of the second gridlocked baseline (jams concentrated on 4-lane primaries, not the injection mesh). Forced-gap values are engineering judgement matching manually-metered operation (JICA 2012: 9 of 10 study junctions police-controlled; IOE 2014 documents non-lane-based operation), not locally estimated ★ | sweep jmIgnoreFoeProb 0.1/0.4 and tau ±0.2 in robustness; replace with locally estimated gap-acceptance if field data ever surfaces. PCU remains the separate accounting weight for count comparison (§3) |
| A13 | Lateral collisions are logged, not enforced as vehicle removals (`--collision.action warn`, with `collision.mingap-factor 0`) | with the sublane model active, a 63%-motorcycle fleet registers continuous minor lateral overlaps; SUMO's default response deletes the vehicle, which removed 96,225 of 145,191 teleporting vehicles in the third baseline and drove modeled volumes to 5-10% of counts. Motorcycles in Kathmandu routinely pass within centimetres, so the overlap is a model artefact of representing them as rectangles, not a crash ★ | verification run 08:00-09:30 with the setting: teleports 145,191 -> 4,628, insertion 53% -> 75%, mean speed 3.9 -> 8.6 m/s. Sensitivity: widen motorcycle minGapLat to the A11 upper end (0.41 m) and confirm results are unchanged |
