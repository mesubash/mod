...
| (live pages, not archived) | DoR SSRN **hourly detail**: per-hour, per-direction, per-class counts for the survey days behind each station-year AADT row. Source of the measured A1 departure profile (`pipeline/dor_hourly.py` → `data/processed/hourly_profile.parquet`) | `traffic_controller/get_detail/<location>/<id>`, e.g. [Manohara Bridge/1582](https://ssrn.dor.gov.np/traffic_controller/get_detail/Manohara%20Bridge/1582) (retrieved 2026-08-16) |
